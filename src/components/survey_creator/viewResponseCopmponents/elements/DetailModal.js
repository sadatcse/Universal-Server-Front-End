...
export default function DetailModal({setIsOpenModal, responseData}) {
  ...
  console.log(Object.keys(responseData.info))
  console.log(Object.values(responseData.info))
  return (
    <div className="fixed left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 py-10">
            <div className={`max-h-full w-full max-w-[800px] overflow-y-auto sm:rounded-2xl bg-white pt-12 p-8 transition-all duration-500 relative`} >
               ...
               <h2 className='font-bold text-3xl border-b-2 border-stone-400'>Customer Satisfaction Survey</h2>
                ...
                {
                  Object.keys(responseData?.info).map((key, idx, array)=> (

                    <p key={idx} className='text-lg btn btn-outline hover:bg-transparent h-fit hover:text-neutral-700 cursor-auto select-text'><b>{key}:</b>{responseData?.info[key]}</p>
                  ))
                }
