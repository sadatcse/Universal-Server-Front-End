    ...
    <div class="p-4 py-8 relative w-full">
      ...
      200" onSubmit={onSubmit}>
        ...
        <div className="flex items-center">
          <input class="title bg-white shadow-md p-2 mb-4 outline-none rounded w-[90%]" spellcheck="false" id="level" placeholder="Add Level" type="text" required name="question" ref={levelInput} onKeyUp={(e) => onEnter(e, "level")} />
        ...
        <div className="flex shrink-0 overflow-x-auto  p-3 border border-neutral-800 rounded-2xl mb-3 gap-3">
          {
            levelNames && levelNames.map((name, idx) => (
              <div key={idx} className="relative p-2 rounded-xl border bg-stone-200 pr-6">{name[0]}
        ...
        <div className="flex w-full justify-stretch rounded-lg overflow-hidden gap-3">
          ...
          <from className="space-y-4 border-2 rounded-lg border-neutral p-3 md:w-1/2 w-full" >
            ...
            <input class="title bg-white shadow-md p-2 mb-4 outline-none rounded" spellcheck="false" id="title" placeholder="Add Option" type="text" required ref={optionInput} onKeyUp={(e) => onEnter(e, "option")} />
        ...
        <div class="flex justify-end mt-4">
          <div class="btn p-1 px-4 font-semibold cursor-pointer hover:bg-blue-200 hover:text-neutral-800 border-none text-gray-200 ml-2 bg-neutral-800" onClick={onSave}>Save Question</div>
        </div>
      </form>
    </div>
  )
