...
function RatingScaleQuestions({ setShowSurveyForm, setSurveyQuestions, setOpenAddQuestionModal, currentQuestion, setCurrentQuestion }) {
  ...
  useEffect(() => {
    ...
    const newObject = { questionType: "linear_scale", question: questionInput, options: [...Array(parseInt(options)).keys()] , answer: "" }
    
    setCurrentQuestion(newObject)
  }, [setCurrentQuestion, questionInput, options])
  ...
  return (
    <div class="p-4 py-8 relative">
      ...
      <form class="editor mx-auto w-full flex flex-col text-gray-800  rounded-md shadow-xl p-4 bg-stone-200" onSubmit={onSubmit}>
        ...
        <select className="select select-bordered select-lg w-full" value={options} onChange={(e) => setOptions(e.target.value)} >
