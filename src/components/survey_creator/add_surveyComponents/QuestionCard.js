    ...
    return (
        <div className="rounded-lg border-2 text-neutral-800 text-4xl border-stone-400 h-28 p-2 pr-6 relative "   >
            ...
            <div className='absolute top-0 right-0 w-8 h-full flex justify-start items-center flex-col py-2 gap-2' >
                ...
                <button className="p-1 text-[12px] text-neutral bg-white border rounded-full hover:bg-blue-500 hover:text-white" onClick={() => { setShowSurveyForm(true); setOpenAddQuestionModal(item?.type) }}><FaEdit /></button>
                <button className="p-1 text-[12px] text-neutral bg-white border rounded-full hover:bg-blue-500 hover:text-white" onClick={() => { setShowSurveyForm(true); setOpenAddQuestionModal(item?.type) }}><TbEyeglass2 /></button>
