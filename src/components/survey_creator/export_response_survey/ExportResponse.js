...
function ExportResponse() {
  ...
  const [exportSurveyData, setExportSurveyData] = useState(surveyData)
  return (
    <>
    <ExportResponseHeader setExportSurveyData={setExportSurveyData} surveyData={surveyData} />
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-4 gap-6">
      {
        exportSurveyData && exportSurveyData.map((data, idx)=> (
          <ExportResponseCard data={data} key={idx} setOpenModal={setOpenModal} />
