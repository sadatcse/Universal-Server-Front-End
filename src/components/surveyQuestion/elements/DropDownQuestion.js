...
function DropDownQuestion({ question, setQuestions, isViewResult }) {
  ...
  const dropdownStyle =
    searchValue?.length > 0 ? { padding: "8px" } : { padding: "0px" };
  return (
    <div
      onClick={onObserver}
      className={isViewResult ? "pointer-events-none" : ""}
    >
      ...
      <div className="mx-auto w-1/2 relative">
        <input
          ...
          className="input input-bordered input-success w-full mx-auto"
          onFocus={() => {
            setSearchValue(question.options);
            setSelectValue("");
          }}
        ...
        <ul
          className="shadow menu dropdown-content z-[1] bg-success gap-3 rounded-box w-full max-h-40 overflow-auto flex-nowrap transition-all duration-100 absolute top-full left-0 mt-3 p-6"
          style={dropdownStyle}
        >
          {searchValue?.map((name, idx) => (
            <li
              ...
              className="text-neutral bg-white shadow-sm rounded-md hover:bg-neutral-800 hover:text-white"
            >
              <a>{name}</a>
            </li>
          ))}
